...
const icons = {
  ...
  AIAssistant: <Mic className="w-5 h-5" />,
};
...
const Header = ({ onMenuToggle, pages, activePage, onPageChange }) => {
  ...
  return (
    <motion.header
      ...
      className="fixed w-full z-50 bg-gradient-to-br from-purple-900 via-black to-purple-800 text-white p-4 flex items-center justify-between shadow-lg"
    ...
    >
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="hidden md:flex items-center space-x-4 text-slate-300">
          <a
            href="https://facebook.com/profile.php?id=61566909757271"
            target="_blank"
        ...
        {currentUser ? (
          ...
          <Link href="/login">
            <motion.button
              className="bg-transparent hover:bg-slate-700 p-2 rounded-full cursor-pointer transition-colors"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
